...
const HistoryTab: React.FC = () => {
  ...
  const fetchSchedules = async () => {
    ...
    try {
      ...
      if (response && response.success && response.data) {
        ...
        setTotalPages(1);
        openSnackbar({
          text: response?.message || 'Không thể tải dữ liệu lịch sử',
          type: 'error'
        });
      }
    } catch (error) {
  ...
  const handleViewDetail = (scheduleId: number) => {
    console.log('Xem chi tiết lịch sử:', scheduleId);
  };

  const handleExportReport = (scheduleId: number) => {
    console.log('Xuất báo cáo:', scheduleId);
    openSnackbar({
      text: `Đang xuất báo cáo cho môn học ID: ${scheduleId}`,
      type: 'success'
    });
  };
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <Box className={`${config.bg} ${config.text} px-2 py-1 rounded-md text-xs font-medium`}>
        {config.label}
      </Box>
    );
  };
      ...
      <Box className="p-3 bg-white border-b border-gray-100">
        <Box className="flex items-center gap-3">
          <Box className="flex-1">
            <Input
              placeholder="Tìm kiếm lịch sử giảng dạy..."
      ...
      <List className="bg-white">
        {loading ? (
          Array.from({ length: 5 }).map((_, index) => (
            <List.Item key={`history-skeleton-${index}`} className="border-b border-gray-100">
              <Box className="p-3 animate-pulse">
                ...
                <Box className="flex gap-2 mb-3">
                  ...
                  <Box className="h-6 bg-gray-200 rounded w-24"></Box>
          ...
          <List.Item>
            <Box className="flex justify-center p-4">
              <Text>Không có lịch sử giảng dạy</Text>
            </Box>
          </List.Item>
        ) : (
          schedules.map((schedule) => (
            <List.Item key={`history-${schedule.id}`} className="border-b border-gray-100">
              <Box className="p-3">
                <Box className="flex justify-between items-start mb-2">
                  <Box className="flex-1">
                ...
                <Box className="flex flex-wrap gap-2 mb-3">
                  ...
                  {/* Status Badge - randomly assign for demo */}
                  {getStatusBadge(['completed', 'substitute', 'cancelled', 'absent'][Math.floor(Math.random() * 4)])}
                </Box>
                ...
                <Box className="flex gap-2">
                  ...
                  >
                    <Icon icon="zi-eye" className="mr-1" />
                    Xem chi tiết
                  </Button>
                  ...
                  <Button
                    size="small"
                    variant="tertiary"
                    onClick={() => handleExportReport(schedule.id)}
                    className="px-3"
                  >
